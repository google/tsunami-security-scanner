...
public final class DefaultTsunamiSocketFactory implements TsunamiSocketFactory {
  ...
  public DefaultTsunamiSocketFactory(
    ...
    checkArgument(
        !defaultConnectTimeout.isNegative(), "Connect timeout cannot be negative");
    checkArgument(
        !defaultReadTimeout.isNegative(), "Read timeout cannot be negative");
  ...
  @Override
  public Socket createSocket(InetAddress address, int port, Duration timeout)
      throws IOException {
  ...
  public Socket createUnconnectedSocket() throws IOException {
    ...
    logger.atFine().log(
        "Created unconnected socket with read timeout %s", defaultReadTimeout);
    return socket;
  }
  ...
  public SSLSocket createSslSocket(
    ...
    configureAndConnect(plainSocket, new InetSocketAddress(host, port), connectTimeout, readTimeout);

    SSLSocket sslSocket =
        (SSLSocket) sslSocketFactory.createSocket(plainSocket, host, port, true);
  ...
  public SSLSocket createSslSocket(
    ...
    // Create a plain socket first, connect with timeout, then wrap with SSL
    Socket plainSocket = socketFactory.createSocket();
    configureAndConnect(plainSocket, new InetSocketAddress(address, port), connectTimeout, readTimeout);
  ...
  public SSLSocket wrapWithSsl(Socket socket, String host, int port, boolean autoClose)
    ...
    SSLSocket sslSocket =
        (SSLSocket) sslSocketFactory.createSocket(socket, host, port, autoClose);
